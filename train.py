...
from pre_data import process_math23k, process_math23k_for_rnn
...
class GTS_Trainer(object):
    ...
    def init_model(self, args, vocab_size):
        # model
        self.encoder=EncoderSeq(args,vocab_size).to(self.device)
        self.predict = Prediction(args).to(self.device)
        self.generate = GenerateNode(args).to(self.device)
        self.merge = Merge(args).to(self.device)
        # optimizer
    ...
    def build_dataset(self, random_seed):
        # data processing
        processed_datas, temp_g, copy_nums = process_math23k_for_rnn()
        # build dataset and vocab
        self.data_set = DataSet(processed_datas, self.device, random_seed)
        self.data_set.build_vocab([], temp_g, copy_nums)
        self.args.input_size = len(temp_g)
        self.args.op_nums = self.data_set.vocab.op_nums
    ...
    def run_train(self, src, mask,num_pos,num_mask,unk,num_start,input_length,\
                    target_length=None,target=None,nums_stack_batch=None,USE_CUDA=False):
        max_input_len=input_length.max()
        mask=mask[:,:max_input_len]
        src=src.transpose(0,1)
        encoder_outputs,problem_output=self.encoder(src, input_length)
        batch_size=encoder_outputs.size(1)
        padding_hidden = torch.FloatTensor(
            [0.0 for _ in range(self.predict.hidden_size)]).unsqueeze(0)
        if target is not None:
    ...
    def eval_batch(self, batch):
        ...
        test_out=self.run_train(batch["question"],batch["ques mask"],batch["num pos"],batch["num mask"],\
                            unk,num_start,batch["ques len"],CUDA_USE)
        val_ac, equ_ac, _, _ = compute_prefix_tree_result(
                test_out, batch["equation"].tolist()[0], self.data_set.vocab,
                batch["num list"][0], batch["num stack"][0])
        return val_ac,equ_ac
